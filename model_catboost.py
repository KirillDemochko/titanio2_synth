...
df.iloc[:, 12] = df.iloc[:, 12].fillna("None").astype(str)
...
for i in range(5):
    ...
    model = CatBoostRegressor(cat_features=[12], verbose=100, random_state=42)
...
pd.DataFrame(y_pred, columns=target_names).to_csv("catboost_predictions.csv", index=False)
